...
def plot_binomial_with_pvalue(n, p, observed_heads):
    ...
    p_value = min(1, 2 * min(p_value_left, p_value_right))
    ...
    if observed_heads <= mean:
        plt.fill_between(k[:observed_heads+1], 0, binomial[:observed_heads+1], alpha=0.5, color='r')
        plt.fill_between(k[int(2*mean-observed_heads):], 0, binomial[int(2*mean-observed_heads):], alpha=0.5, color='r')
    else:
        plt.fill_between(k[:int(2*mean-observed_heads)+1], 0, binomial[:int(2*mean-observed_heads)+1], alpha=0.5, color='r')
        plt.fill_between(k[observed_heads:], 0, binomial[observed_heads:], alpha=0.5, color='r')
    
    plt.axvline(observed_heads, color='r', linestyle='dotted', linewidth=2)
    plt.text(observed_heads+1, plt.ylim()[1]*0.6, f'Observed: {observed_heads}\np-value: {p_value:.6f}', color='black')
    
    # Add arrows pointing to the p-value areas
    if observed_heads <= mean:
        plt.annotate('p-value area', xy=(35,0.00),
                     xytext=(30, 0.03), arrowprops=dict(facecolor='red', shrink=0.05), color='r')
        
        plt.annotate('p-value area', xy=(70,0.00), 
                     xytext=(70, 0.03), arrowprops=dict(facecolor='red', shrink=0.05), color='r')
    else:
        plt.annotate('p-value area', xy=(35, binomial[int(mean-observed_heads/2)]), 
                     xytext=(30, 0.03), arrowprops=dict(facecolor='red', shrink=0.05), color='r')
        plt.annotate('p-value area', xy=(65, binomial[int((observed_heads+n)/2)]), 
                     xytext=(65,0.03), arrowprops=dict(facecolor='red', shrink=0.05), color='r')
    
    # Add horizontal green line for PDF at observed_heads
    pdf_value = binom.pmf(observed_heads, n, p)
    plt.axhline(y=pdf_value, color='g', linestyle='--', linewidth=2)
    #plt.text(n*0.8, pdf_value, f'PDF at {observed_heads}: {pdf_value:.6f}', color='g', verticalalignment='bottom')
...
p = float(input('Enter the probability of heads (p) for each toss: '))
